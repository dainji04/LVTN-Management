...
class Group10ApplicationTests {
    ...
    @Autowired
    group_10.group._0.repository.UsersRepository usersRepository;

    @Autowired
    group_10.group._0.repository.BaoCaoRepository baoCaoRepository;
    ...
    @Test
    @Order(1)
    @DisplayName("Setup – Lấy token cho User A, B, C và cấp quyền Admin")
    void setup_getTokens() throws Exception {
        // Cấp quyền Admin cho User A trước khi lấy token để chứa Role ADMIN
        group_10.group._0.entity.Users userAObj = usersRepository.findByEmail(EMAIL_A).orElse(null);
        if (userAObj != null) {
            userAObj.setRole(group_10.group._0.Enum.ROLE.ADMIN);
            usersRepository.save(userAObj);
        }
...
//    }
    ...
    @Test
    @Order(80)
    @DisplayName("TC-RP01 ✅ User B báo cáo User C")
    void tc_RP01_taoBaoCao() throws Exception {
        // Dọn dẹp các báo cáo cũ để tránh lỗi "đối tượng đã được báo cáo"
        ...
        reportId = objectMapper.readTree(
                result.getResponse().getContentAsString())
                .path("data").path("id").asInt();
        
        System.out.println(">>> Đã tạo báo cáo ID: " + reportId);
    }

    @Test
    @Order(81)
    @DisplayName("TC-RP02 ✅ Admin (User A) lấy danh sách báo cáo")
    void tc_RP02_adminGetReports() throws Exception {
    ...
    @Test
    @Order(82)
    @DisplayName("TC-RP03 ✅ Admin (User A) duyệt báo cáo -> Khoá User C")
    void tc_RP03_adminDuyetBaoCao() throws Exception {
        String body = """
                {
                  "trangThai": "DA_XU_LY"
                }
        ...
        mockMvc.perform(put("/reports/" + reportId)
                ...
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.trangThai").value("DA_XU_LY"));
    }

    @Test
    @Order(90)
    @DisplayName("TC-DB01 ✅ Admin Dashboard: Thống kê tổng quan")
    void tc_DB01_dashboardOverview() throws Exception {
        MvcResult result = mockMvc.perform(get("/admin/dashboard/overview")
                        .header("Authorization", "Bearer " + tokenA))
                .andExpect(status().isOk())
                .andReturn();
        ...
        System.out.println("===========================================");
        System.out.println(">>> DỮ LIỆU DASHBOARD TỔNG QUAN:");
        System.out.println(objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(objectMapper.readTree(responseBody)));
        System.out.println("===========================================");
    }

    @Test
    @Order(91)
    @DisplayName("TC-DB02 ✅ Admin Dashboard: Biểu đồ bài viết 7 ngày")
    void tc_DB02_dashboardPostChart() throws Exception {
        MvcResult result = mockMvc.perform(get("/admin/dashboard/stats/posts")
                        .header("Authorization", "Bearer " + tokenA))
                .andExpect(status().isOk())
                .andReturn();
        ...
        String responseBody = result.getResponse().getContentAsString();
        System.out.println("===========================================");
        System.out.println(">>> DỮ LIỆU DASHBOARD BIỂU ĐỒ BÀI VIẾT:");
        System.out.println(objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(objectMapper.readTree(responseBody)));
        System.out.println("===========================================");
    }
    ...
    @Test
    @Order(92)
    @DisplayName("TC-DB03 ✅ Admin Dashboard: Bảng xếp hạng")
    void tc_DB03_dashboardLeaderboards() throws Exception {
        MvcResult result = mockMvc.perform(get("/admin/dashboard/leaderboards")
                        .header("Authorization", "Bearer " + tokenA))
                .andExpect(status().isOk())
                .andReturn();
        ...
        String responseBody = result.getResponse().getContentAsString();
        System.out.println("===========================================");
        System.out.println(">>> DỮ LIỆU DASHBOARD BẢNG XẾP HẠNG:");
        System.out.println(objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(objectMapper.readTree(responseBody)));
        System.out.println("===========================================");
    }
    @Test
    @Order(93)
    @DisplayName("TC-RP04 ❌ Báo cáo trùng lặp -> Lỗi REPORT_IS_EXISTED")
    void tc_RP04_taoBaoCaoTrungLap() throws Exception {
        // User B tạo báo cáo User C GIỐNG HỆT lần 1
        String body = String.format("""
                {
                  "loaiDoiTuong": "NGUOI_DUNG",
                  "idDoiTuong": %s,
                  "lyDo": "Spam tin nhắn lần 2"
                }
                """, extractUserId(tokenC));
    ...
    @Test
    @Order(94)
    @DisplayName("TC-RP05 ❌ User thường xem danh sách báo cáo -> Lỗi UNAUTHENTICATED (401)")
    void tc_RP05_userXemDanhSachBaoCao() throws Exception {
        // User B gọi API của Admin
        mockMvc.perform(get("/reports")
                        .header("Authorization", "Bearer " + tokenB))
                .andDo(print())
                .andExpect(status().isUnauthorized())
    ...
    @Test
    @Order(95)
    @DisplayName("TC-RP06 ❌ Admin xử lý báo cáo không tồn tại -> Lỗi REPORT_NOT_EXISTED")
    void tc_RP06_xuLyBaoCaoKhongTonTai() throws Exception {
        String body = """
                {
                  "trangThai": "DA_XU_LY"
                }
        ...
        mockMvc.perform(put("/reports/999999")
                        .header("Authorization", "Bearer " + tokenA)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andDo(print())
    ...
    @Test
    @Order(96)
    @DisplayName("TC-RP07 ✅ Admin mở khoá người dùng")
    void tc_RP07_moKhoaNguoiDung() throws Exception {
        mockMvc.perform(put("/reports/unlock")
                        .param("loaiDoiTuong", "NGUOI_DUNG")
                        .param("idDoiTuong", String.valueOf(extractUserId(tokenC)))
                        .header("Authorization", "Bearer " + tokenA))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Đã mở khoá đối tượng thành công"));
    }

    @Test
    @Order(97)
    @DisplayName("TC-DB04 ❌ User thường vào Dashboard -> Lỗi UNAUTHENTICATED (401)")
    void tc_DB04_userVaoDashboard() throws Exception {
    ...
    @Test
    @Order(98)
    @DisplayName("TC-RP08 ✅ Báo cáo Bài Viết, Duyệt và Mở khoá")
    void tc_RP08_baoCaoBaiViet() throws Exception {
        // 1. Tạo một bài viết bởi User A
        String postBody = String.format("""
                {
                  "maNguoiDung": %s,
                  "noiDung": "Bài viết vi phạm chính sách",
                  "danhSachAnh": []
                }
                """, extractUserId(tokenA));

        MvcResult postResult = mockMvc.perform(post("/bai-viet")
                        .header("Authorization", "Bearer " + tokenA)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(postBody))
                .andExpect(status().isCreated())
        ...
        Integer baiVietId = objectMapper.readTree(postResult.getResponse().getContentAsString())
                .path("data").path("maBaiViet").asInt();

        // 2. User B báo cáo bài viết vừa tạo
        String reportBody = String.format("""
                {
                  "loaiDoiTuong": "BAI_VIET",
                  "idDoiTuong": %s,
                  "lyDo": "Bài viết chứa nội dung phản cảm"
                }
                """, baiVietId);

        MvcResult reportResult = mockMvc.perform(post("/reports")
                        .header("Authorization", "Bearer " + tokenB)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(reportBody))
                .andExpect(status().isOk())
        ...
        Integer rpBaiVietId = objectMapper.readTree(reportResult.getResponse().getContentAsString())
                .path("data").path("id").asInt();

        // 3. Admin (User A - đã cấp quyền admin) duyệt báo cáo -> Khoá bài viết
        mockMvc.perform(put("/reports/" + rpBaiVietId)
                        .header("Authorization", "Bearer " + tokenA)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                        {
        ...
        // 4. Admin mở khoá Bài viết
        mockMvc.perform(put("/reports/unlock")
                        .param("loaiDoiTuong", "BAI_VIET")
                        .param("idDoiTuong", String.valueOf(baiVietId))
                        .header("Authorization", "Bearer " + tokenA))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Đã mở khoá đối tượng thành công"));
    }
    ...
    @Test
    @Order(99)
    @DisplayName("TC-RP09 ✅ Báo cáo Nhóm, Duyệt và Mở khoá")
    void tc_RP09_baoCaoNhom() throws Exception {
        // 1. Tạo một nhóm bởi User A
        String groupBody = """
                {
                  "tenNhom": "Nhóm vi phạm cộng đồng",
                  "moTa": "Test nhóm bị báo cáo",
                  "loaiNhom": "PUBLIC",
                  "canDuyetBaiDang": false
                }
                """;

        MvcResult groupResult = mockMvc.perform(post("/group")
                        .header("Authorization", "Bearer " + tokenA)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(groupBody))
                .andExpect(status().isOk())
        ...
        Integer groupId = objectMapper.readTree(groupResult.getResponse().getContentAsString())
                .path("data").path("id").asInt();

        // 2. User C báo cáo nhóm
        String reportBody = String.format("""
                {
                  "loaiDoiTuong": "NHOM",
                  "idDoiTuong": %s,
        ...
        Integer rpGroupId = objectMapper.readTree(reportResult.getResponse().getContentAsString())
                .path("data").path("id").asInt();

        // 3. Admin duyệt báo cáo -> Khoá nhóm
        mockMvc.perform(put("/reports/" + rpGroupId)
                        .header("Authorization", "Bearer " + tokenA)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                        {
        ...
        // 4. Admin mở khoá Nhóm
        mockMvc.perform(put("/reports/unlock")
                        .param("loaiDoiTuong", "NHOM")
                        .param("idDoiTuong", String.valueOf(groupId))
                        .header("Authorization", "Bearer " + tokenA))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Đã mở khoá đối tượng thành công"));
    }
