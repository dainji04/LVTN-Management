                            ...
                            >
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <button
                                            @click.stop="handleDelete(user.maNguoiDung, user.ho, user.ten)"
...
const fetchUsers = async () => {
    ...
    try {
        ...
        if (response.data.code === 200 && response.data.data) {
            ...
            error.value = response.data.message || 'Không thể tải danh sách người dùng';
            notificationHelper('error', error.value);
        }
    } catch (err: any) {
        error.value = err.response?.data?.message || err.message || 'Đã xảy ra lỗi khi tải danh sách người dùng';
        notificationHelper('error', error.value);
...
const handleDelete = async (userId: number, ho: string, ten: string) => {
    if (!confirm(`Bạn có chắc chắn muốn xóa người dùng "${ho} ${ten}"? Hành động này không thể hoàn tác.`)) {
        return;
    }
    ...
    try {
        ...
        const errorMessage = err.response?.data?.message || err.message || 'Đã xảy ra lỗi khi xóa người dùng';
